...
class DataHandler {
    ...
    async loadRegionalData(region) {
        try {
            if (!region) return [];
            const response = await fetch(`/api/data/regional?region=${encodeURIComponent(region)}`);
            if (!response.ok) throw new Error('Errore HTTP');
            const result = await response.json();
            if (!result.success || !result.data) return [];
            // Filtra per la data selezionata
            return result.data.filter(record => record.data === this.selectedDate);
        } catch (e) {
            console.error('Errore nel caricamento dati regionali:', e);
            return [];
        }
    }

    async loadProvincialData(province) {
        try {
            if (!province) return [];
            const response = await fetch(`/api/data/provincial?province=${encodeURIComponent(province)}`);
            if (!response.ok) throw new Error('Errore HTTP');
            const result = await response.json();
            if (!result.success || !result.data) return [];
            // Filtra per la data selezionata
            ...
            console.error('Errore nel caricamento dati provinciali:', e);
            return [];
        }
    }
}
